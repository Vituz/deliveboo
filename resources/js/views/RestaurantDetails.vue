  ...
  <div class="single_rest container" v-if="restaurant">
    ...
    <div class="d-flex flex-wrap">
        ...
        >
          ...
          <div class="right_card d-flex flex-column col-sm-8 p-3">
            ...
            <div
              ...
              "
              @click="
                addItemToCart(dish.user_id, dish.id, dish.name, dish.price)
              "
              type="button"
            >
      ...
      <div class="d-flex flex-column content-section col-md-4 cart">
        ...
        <div
          class="d-flex flex-column cart-items border border-success p-2 mb-2"
        >
          <div class="cart-row" v-for="item in cart" :key="item.id">
            <div class="cart-item cart-column mb-2">
              <span class="cart-item-title text-uppercase">{{
                item.item_name
              }}</span>
              <span class="cart-price cart-column text-align-right"
                >{{ item.item_price }} €</span
              >
            </div>
            <div
              class="cart-quantity cart-column d-flex align-items-center mb-2"
            >
              <button
                class="btn remove_btn btn-sm mr-3"
        ...
        <a
          :href="url"
          class="btn buy_btn btn-purchase text-uppercase text-white"
  ...
  methods: {
    addItemToCart(restaurant, id, title, price) {
      ...
      if (carStored != 0 && carStored[0].user_id != restaurant) {
        alert(
          "concludi l'ordine dal ristorante precedente o svuota il carrello prima di procedere a un nuovo ordine"
        );
      } else {
